  ...
  (let [FOUND-FAILED-CHECK 	true
    ...
    (defn failed-check [found-error? url-check]
      (if-not (:check-accurate url-check)
              (reduced FOUND-FAILED-CHECK)                        ; return true early once a failed check is found
               ALL-ACCURATE-CHECKS))
...
(defn build-db [table-name pages-to-check db-type config-file environment-utilize]
  (let [ the-config (make-config db-type config-file environment-utilize)
        ...
        sms-data (compact-hash till-username till-api-key phone-numbers heroku-app-name)
