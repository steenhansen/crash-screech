(ns sff-audio.mongo-db
  ...
  (:require [sff-audio.check-data :refer [prepare-data trunc-string]])
...
(defn mongolabs-build
  ...
  (let [mongodb-user-pass-uri (:MONGODB_URI mongolabs-config)
        ...
        my-delete-table      (fn delete-table [] (mong-coll/drop db-handle my-collection))

        my-purge-table    (fn purge-table [] (mong-coll/purge-many db-handle [my-collection]))
